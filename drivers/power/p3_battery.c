...
/* fullcharge and discharge comp: During the small window between
...
*/
	if ((battery->previous_cable_status == CHARGER_AC) &&
		...
		pr_info("%s: Keep SOC tied to 100 for UI.\n", __func__);
		fg_set_full_charged();
		/* Delay for new Full Cap to settle down */
		mdelay(100);
	}
...
static irqreturn_t p3_TA_interrupt_handler(int irq, void *arg)
{
	...
	__cancel_delayed_work(&battery->TA_work);
...
static int p3_get_bat_level(struct power_supply *bat_ps)
{
	...
	if ((battery->info.charging_source == CHARGER_AC ||
		...
		if (is_over_abs_time(battery)) {
			...
			pr_info("%s: charging time is over.\n", __func__);
...
static void p3_set_chg_en(struct battery_data *battery, int enable)
{
	...
	if (enable) {
		...
		/* charging stopped by cable disconnected*/
...
static int p3_get_bat_temp(struct power_supply *bat_ps)
{
	...
	if (battery_temp >= battery->pdata->temp_high_threshold) {
		if (health != POWER_SUPPLY_HEALTH_OVERHEAT &&
				health != POWER_SUPPLY_HEALTH_UNSPEC_FAILURE) {
			p3_temp_control(battery, POWER_SUPPLY_HEALTH_OVERHEAT);
			update = 1;
		}
	} else if (battery_temp <= battery->pdata->temp_high_recovery &&
			battery_temp >= battery->pdata->temp_low_recovery) {
		if (health == POWER_SUPPLY_HEALTH_OVERHEAT ||
				health == POWER_SUPPLY_HEALTH_COLD) {
			p3_temp_control(battery, POWER_SUPPLY_HEALTH_GOOD);
			update = 1;
		}
	} else if (battery_temp <= battery->pdata->temp_low_threshold) {
...
static struct device_attribute p3_battery_attrs[] = {
	...
	SEC_BATTERY_ATTR(batt_reset_cap),
