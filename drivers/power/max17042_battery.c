...
int fg_reset_soc(void)
{
	...
	pr_info("%s : Additional Step - VfSOC(%d.%d), New SOC(%d.%d),"
							" New RemCAP(%d)\n",
					__func__, (fg_vfsoc/100),
					(fg_vfsoc%100), (new_soc/100),
					(new_soc%100),
...
static int fg_i2c_remove(struct i2c_client *client)
{
	...
	free_irq(gpio_to_irq(chip->client->irq), NULL);
...
static
int fg_i2c_probe(struct i2c_client *client,  const struct i2c_device_id *id)
{
