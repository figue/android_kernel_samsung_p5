...
static int firmware_update(struct melfas_ts_data *ts)
{
	...
	//	printk("<TSP> CORE_VERSION : 0x%2X\n",version_info);
	  if((ts->touch_id == 0 && version_info < MEL_CORE_FW_VER)
		|| (ts->touch_id == 1 && version_info < ILJ_CORE_FW_VER)
		|| version_info==0xFF)
	   dl_enable_bit |= 0x01;
	 }
	...
	//	printk("<TSP> PRIVATE_CUSTOM_VERSION : 0x%2X\n",version_info);
	  if((ts->touch_id == 0 && version_info < MEL_PRIVATE_FW_VER)
		|| (ts->touch_id == 1 && version_info < ILJ_PRIVATE_FW_VER)
		|| version_info==0xFF)
	   dl_enable_bit |= 0x02;
	 }
	 ...
	 //  printk("<TSP> PUBLIC_CUSTOM_VERSION : 0x%2X\n",version_info);
	   if((ts->touch_id == 0 && version_info < MEL_PUBLIC_FW_VER)
		 || (ts->touch_id == 1 && version_info < ILJ_PUBLIC_FW_VER)
		 || version_info==0xFF)
		dl_enable_bit |= 0x04;
	  }
	  if((ts->touch_id == 0 && (val < MELFAS_FW_VER || val > 0x50 ))
...
// ISC mode start
	...
	input_mt_init_slots(input, P5_MAX_TOUCH);
