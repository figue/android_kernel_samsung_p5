...
#else
	val |= UTMIP_HSDISCON_LEVEL(~0) | UTMIP_HSDISCON_LEVEL_MSB;
#endif
...
static void vbus_enable(struct tegra_usb_phy *phy)
{
	...
	if (usb_phy_data[phy->instance].vbus_en)
		usb_phy_data[phy->instance].vbus_en(1, true);
#endif
	if (gpio == -1)
		return;
...
#ifdef CONFIG_MACH_SAMSUNG_VARIATION_TEGRA
	if (usb_phy_data[phy->instance].vbus_en)
		usb_phy_data[phy->instance].vbus_en(1, false);
#endif
	if (gpio == -1)
		return;
...
static int uhsic_phy_power_on(struct tegra_usb_phy *phy, bool is_dpd)
{
	...
	utmi_wait_register(base + USB_PORTSC1, USB_PORTSC1_PSPD(3),
						USB_PORTSC1_PSPD(3));
	pr_info("%s : check register, hsic portsc1 : 0x%08x\n", __func__,
