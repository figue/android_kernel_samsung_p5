...
void __init tegra_p3_reserve(void)
{
    ...
    /* I'm assuming the first 256MB chunk comes from the graphics card/camera memory
       It appears that all "math manipulation" must be done using the constants 256 16 8 1 in either addition/subtraction/multiplication combos
       The current change will free up 80MB (5*16) from the assumed graphics card/camera to main memory. */
	tegra_reserve(SZ_256M - 5 * SZ_16M, SZ_8M + SZ_1M, SZ_16M);
